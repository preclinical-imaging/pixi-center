// Submit a Dataset — public dataset proposal form, its own top-level page
// (previously a tab inside the ui_kits/web_app SPA). Uses the same TopNav/
// Footer as the other public pages (../home/site-chrome.jsx) and the form
// primitives from ../home/ui.jsx.
//
// POSTs to /api/submissions on the same origin (served by
// ui_kits/web_app/server.js — see that file's "Static file serving" section;
// ROOT there is the repo root, so this page and that endpoint are already
// same-origin regardless of which top-level page the fetch comes from).
//
...
const SubmitDatasetForm = ({ onSubmitted }) => {
  ...
  return (
    <section style={{ padding: "40px 40px 96px", maxWidth: 900, margin: "0 auto" }}>
      <Eyebrow mono>Contribute</Eyebrow>
      <h1 style={{
        margin: "14px 0 8px", fontFamily: "var(--font-display)",
        fontSize: 36, fontWeight: 600, letterSpacing: "-0.02em",
        color: "var(--fg-1)",
      }}>Submit a dataset</h1>
      <p style={{ margin: "0 0 28px", fontSize: 15, color: "var(--fg-2)", lineHeight: 1.55, maxWidth: 620 }}>
...
const SubmitDatasetThanks = () => (
  <section style={{ padding: "40px 40px 96px", maxWidth: 900, margin: "0 auto" }}>
    <div style={{
      ...
      <a href="../ui_kits/web_app/index.html"><Button variant="primary">Browse datasets</Button></a>
    </div>
  </section>
);
...
const SubmitDatasetPage = () => {
  ...
  return (
    <div className="pixi-root" data-screen-label="PIXI Center submit dataset">
      <TopNav />
      {submitted ? <SubmitDatasetThanks /> : <SubmitDatasetForm onSubmitted={() => setSubmitted(true)} />}
      <Footer />
    </div>
  );
};

ReactDOM.createRoot(document.getElementById("root")).render(<SubmitDatasetPage />);
